...
class Prachtstueck():
    ...
    def on_enter_init(self):
        ...
        self.communication_queue.send(Message('set_camera', [81]))
    ...
    def on_enter_to_load(self):
        ...
        self.communication_queue.send(Message('drive', [270, -290, 10000])) #- (self.position.z-20)

    def on_enter_insert_load(self):
        print("Nach vorne fahren ca 2cm")
        self.communication_queue.send(Message('drive_x', [50, 3500]))

    def on_enter_grab_load(self):
        print("Wuerfel greifen")
        self.communication_queue.send(Message('setGrabber', [180]))
    ...
    def on_enter_lift_load(self):
        ...
        self.communication_queue.send(Message('drive_z_to_home', [20000]))
    ...
    def on_enter_to_target(self):
        ...
        self.communication_queue.send(Message('drive_x', [(315-self.position.x)*10, 12000]))
    ...
    def on_enter_center_target(self):
        ...
        self.communication_queue.send(Message('drive_x', [(315-self.position.x)*10, 2500]))
    ...
    def on_enter_set_load(self):
        ...
        self.communication_queue.send(Message('stop'))
        self.communication_queue.send(Message('drive', [30, -(self.position.z*10-17), 5000])) #-
    ...
    def on_enter_lift_grabber(self):
        print("Greifer nach oben")
        self.communication_queue.send(Message('drive_z', [300, 20000]))

    def on_enter_fast_to_stop(self):
        print("Berechne restliche Fahrt")
        print("Berechnete Strecke fahren")
        self.communication_queue.send(Message('drive_x', [(315-self.position.x)*10, 30000])) #self.position.x-20
